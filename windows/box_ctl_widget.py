...
class BoxCtlWidget(QtWidgets.QGroupBox):
    def __init__(self, dispatch: Callable, model: BoxModel = None, *args, **kwargs):
        ...
        layout.addWidget(QtWidgets.QLabel("width"), 3, 0)
        self.w_size_x = QtWidgets.QDoubleSpinBox()
        layout.addWidget(self.w_size_x, 3, 1)
        layout.addWidget(QtWidgets.QLabel("height"), 3, 2)
        self.w_size_y = QtWidgets.QDoubleSpinBox()
        layout.addWidget(self.w_size_y, 3, 3)
        layout.addWidget(QtWidgets.QLabel("length"), 3, 4)
        self.w_size_z = QtWidgets.QDoubleSpinBox()
        layout.addWidget(self.w_size_z, 3, 5)

        layout.addWidget(QtWidgets.QLabel("rotate"), 4, 0, 1, 1)
        self.w_rotate = QtWidgets.QSlider(Qt.Horizontal)
        self.w_rotate.setMinimum(0)
        self.w_rotate.setMaximum(360)
        layout.addWidget(self.w_rotate, 4, 1, 1, 3)
    ...
    def store_connect(self):
        ...
        self.w_size_x.valueChanged.connect(self.model_action)
        self.w_size_y.valueChanged.connect(self.model_action)
        self.w_size_z.valueChanged.connect(self.model_action)
        self.w_rotate.valueChanged.connect(self.model_action)
        self.w_deleter.clicked.connect(lambda: self.dispatch(self, None))

    def to_model(self) -> BoxModel:
        color = self.w_pal.color(QPalette.Window)
        return BoxModel(id=self.id,
                        name=self.w_name.text(),
                        translation=Vector3Model(x=self.w_pos_x.value(), y=self.w_pos_y.value(), z=self.w_pos_z.value()),
                        color=RgbaModel(r=color.redF(), g=color.greenF(), b=color.blueF(), a=color.alphaF()),
                        rotate=self.w_rotate.value(),
                        size=Vector3Model(x=self.w_size_x.value(), y=self.w_size_y.value(), z=self.w_size_z.value()),
                        )
    ...
    def reduce(self, model: BoxModel):
        ...
        self.w_size_x.setValue(model.size.x)
        self.w_size_y.setValue(model.size.y)
        self.w_size_z.setValue(model.size.z)

        self.w_rotate.setValue(model.rotate)
        ...
        self.setTitle('box-' + str(model.id))
